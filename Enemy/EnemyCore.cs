...
public class EnemyCore : MonoBehaviour, IDamageable, IPoolable
{
    ...
    public void TakeDamage(int damage, Vector3 hitPoint, Transform damageSource)
    {
        ...
        // Instanciar el indicador de daño si el prefab está asignado
        if (damageIndicatorPrefab != null)
        {
            // Creamos una instancia del prefab en la posición del enemigo (o en el punto de impacto)
            GameObject indicator = ObjectPoolManager.Instance.Spawn(
                "DamageIndicator",
                transform.position + Vector3.up * 1.0f,
                Quaternion.identity
            );
            // Obtenemos el script y lo inicializamos con la cantidad de daño
            DamageIndicator indicatorScript = indicator.GetComponent<DamageIndicator>();
            if (indicatorScript != null)
            {
                indicatorScript.Initialize(damage);
            }
        }
